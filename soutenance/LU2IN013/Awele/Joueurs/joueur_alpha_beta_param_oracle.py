...
COMPTEUR=0 
"""
params_BEG =  [0,0,0,0,0,0,0,0,0]
params_MID = [0,0,0,0,0,0,0,0,0]
params_END =[0,0,0,0,0,0,0,0,0]
...
def estimation(jeu,coup,profondeur,alpha,beta):
    ...
    if (game.finJeu(copie_jeu)): 
        ...
        if (game.getGagnant(copie_jeu) == PLAYER%2+1) :
            return -10000000000
    if  profondeur ==  1 :
        return evaluation(copie_jeu,False)
...
"""
def evaluation(jeu,b): 
    if(b):
        global PLAYER
        PLAYER = jeu[1]
        s = scores(jeu)
        return s 
    return dot(params,scores(jeu))
...
def addparam(i,k):
    global params_BEG , params_MID , params_END , params_TOT , params
    params[i] = params[i] + k
...
def max_value(jeu,profondeur,alpha,beta):
    ...
    for c in game.getCoupsValides(jeu):
        ...
        if ( m >= beta):
            return m
        m = max(estimation(jeu,c,profondeur-1,alpha,beta,False), m)
    return m

def min_value(jeu,profondeur,alpha,beta):
    m = beta
    for c in game.getCoupsValides(jeu):
        m = min(estimation(jeu,c,profondeur-1,alpha,beta,False), m)
        beta = m 
        if( m <= alpha ):
